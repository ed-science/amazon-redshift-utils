...
from reportlab.platypus import PageBreak, TableStyle, Table, Spacer, Image, SimpleDocTemplate, Paragraph, ListFlowable,\
    ListItem
from report_util import styles, build_pdf_tables, df_to_np, first_page, later_pages, hist_gen, sub_yaml_vars
...
def pdf_gen(report, summary=None):
    ...
    with open("report_content.yaml", 'r') as stream:
        ...
        image_path = hist_gen(x_data=report.feature_graph['sec_start'],
        ...
        build_pdf_tables(elems, docs['measure_tables'], report)     # build 5 measure tables all at once
